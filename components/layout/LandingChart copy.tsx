...
import { initializePatterns, MaskConfig, PatternConfig, PatternDirection, PatternRegistry, PatternType } from "@/lib/highcharts/svgPatterns copy";
...
// Updated COMPOSITION_TYPES with simplified pattern references
const COMPOSITION_TYPES: Record<string, {
  name: string;
  description: string;
  order: number;
  style: {
  ...
  cross_layer: {
    ...
    // Blue stripes over gradient background
    patternConfig: {
      defType: 'pattern',
      type: 'colored-hash',
      direction: 'right',
      color: '#94ABD3', // Blue stripes
  ...
  single_l2: {
    ...
    // Don't store the mask ID name but store the type of mask to use
    // We'll look up the actual ID at render time
    maskConfig: {
      defType: 'mask',
      type: 'hash',
      direction: 'right',
...
export default function LandingChart({
  ...
  const customTooltipFormatter = useMemo(() => createTooltipFormatter({
    ...
    colorMap: Object.keys(COMPOSITION_TYPES).reduce((acc, key) => {
      const gradient = COMPOSITION_TYPES[key].gradient;
      if(gradient){
        acc[key] = gradient.stops[0][1];
        return acc;
      }
      const patternConfig = COMPOSITION_TYPES[key].patternConfig;
      if(patternConfig && patternConfig.color){
        acc[key] = patternConfig.color;
        return acc;
      }
      acc[key] = "#000000";
      console.log("key", key, "color", acc[key]);
      return acc;
  ...
  // Simplified getSeriesData function
const getSeriesData = useCallback(
  (name: string, types: string[], data: number[][], chartType: string) => {
    if (name === "")
      return {
        data: [],
        color: ["", ""]
    ...
    const compositionType = COMPOSITION_TYPES[name];
    if (!compositionType) {
      return {
        data: [],
        color: ["", ""]
    ...
    // Get gradient colors from composition type
    const gradient = compositionType.gradient;
    const colors = gradient?.stops.map(stop => stop[1]) || [];
    
    // Time index for X values
    // const timeIndex = types.findIndex(type => type === "unix");
    
    // // Value index for Y values (USD or ETH based on showUsd)
    // let valueIndex = types.includes("usd") && showUsd ? types.indexOf("usd") : types.indexOf("eth");
    // let valueMulitplier = 1;
    
    // Map data points
    // const seriesData = data.map(d => [d[timeIndex], d[valueIndex] * valueMulitplier]);
...
const getPrimaryColor = useCallback((name: string) => {
  const compositionType = COMPOSITION_TYPES[name];
  const gradient = compositionType.gradient;
  if(gradient){
    return gradient.stops[0][1];
  }
  const patternConfig = compositionType.patternConfig;
  if(patternConfig){
    return patternConfig.color;
  }
  return "#000000";
}, []);
  ...
  const options = useMemo((): Highcharts.Options => {
    const dynamicOptions: Highcharts.Options = {
      chart: {
        ...
        events: {
          load: function() {
            // Initialize the pattern registry with all needed patterns
            const registry = initializePatterns(this);
            
            // Store registry in chart options for access in render event
          ...
          render: function() {
            ...
            // Apply masks and patterns to any series that needs them
            this.series.forEach((series, index) => {
              ...
              // Apply mask if maskType is specified
              if (typeConfig.maskConfig) {
                // Create or get the pattern with the requested mask
                const pattern = registry.register(typeConfig.maskConfig);
                
                if (pattern.maskId) {
                  registry.applyMaskToSeries(index, pattern.maskId);
                } else {
                  console.warn(`No mask ID found for series ${series.name}`);
                }
              }
              
              // Apply pattern if specified
              if (typeConfig.patternConfig) {
                // Register a specific pattern for this series
                const pattern = registry.register(typeConfig.patternConfig);

                registry.applyPatternToSeries(index, pattern.patternId);
        ...
        ...filteredData
          .sort((a, b) => {
            if(!["main_l1", "main_l2"].includes(a.name)) {
              return COMPOSITION_TYPES[b.name]!.order - COMPOSITION_TYPES[a.name]!.order;
            }
            const aValue =
          ...
          .map((series: any, i: number) => {
            ...
            const pointsSettings =
              ...
              console.log("series", series.name, "color", getPrimaryColor(series.name));
            ...
            return {
              ...
              color: COMPOSITION_TYPES[series.name]?.gradient || {}, // Use gradient from composition type
              fillColor: COMPOSITION_TYPES[series.name]?.gradient || {}, // Use gradient from composition type
              states: {
                hover: {
                  enabled: true,
                  halo: {
                      ...
                      attributes: {
                        fill:
                          getPrimaryColor(series.name) + "99",
                        stroke:
                          getPrimaryColor(series.name) + "66",
                        "stroke-width": 0,
                      },
                    },
                    brightness: 0.3,
              ...
              marker: {
                lineColor: getPrimaryColor(series.name),
                radius: 2,
                symbol: "circle",
              },
