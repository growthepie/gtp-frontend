...
export const LinkButton = ({ href, children, icon, iconClassName }: LinkButtonProps) => {
  ...
  if(icon) {
    return (
      <Link className="flex py-[2px] px-[3px] bg-color-bg-medium hover:bg-color-ui-hover rounded-full items-center gap-x-[5px] relative top-[1px]" href={href} target={href.includes("http") ? "_blank" : "_self"} onClick={(e) => {
        e.stopPropagation();
        handleClick();
      }}>
  ...
  return (
    <Link className="flex py-[2px] pl-[5px] pr-[3px] bg-color-bg-medium hover:bg-color-ui-hover rounded-full items-center gap-x-[5px] relative top-[1px]" href={href} target={href.includes("http") ? "_blank" : "_self"} onClick={(e) => {
      e.stopPropagation();
      handleClick();
    }}>
      <div className="text-xxs text-nowrap">{children}</div>
