...
import { FloatingBarButton, FloatingBarExpanableLabelButton } from './FloatingBar/FloatingBarButton';
...
import { IS_PRODUCTION, AUTH_SUBDOMAIN } from '@/lib/helpers';
import { useAuth } from '@/contexts/AuthContext';
import EmailAuthInline from '../auth/EmailAuthInline';
...
export default function GlobalFloatingBar() {
  ...
  // Auth state
  const { isAuthenticated, email: userEmail, logout } = useAuth();
  ...
  // check if we're on a protected domain
  useEffect(() => {
  ...
  useEffect(() => {
    if (isAuthenticated && isProtectedDomain) {
  ...
  }, [isAuthenticated, isProtectedDomain]);
  ...
  return (
    <>
      {/* Background gradient */}
      <div className={`fixed z-global-search-backdrop bottom-[-200px] md:bottom-auto md:top-[0px] w-full max-w-[1920px] px-0 md:px-[13px] md:-mx-[5px] transition-[margin] duration-sidebar ease-sidebar flex justify-center ${!isAuthenticated && isProtectedDomain ? 'opacity-0' : 'opacity-100'}`}>
      ...
      {/* Main floating bar */}
      <div
        className={`fixed z-global-search left-0 right-0 flex justify-center w-full pointer-events-none transition-all duration-[1000ms] ease-out ${!isAuthenticated && isProtectedDomain
            ? 'translate-y-[calc(+84px-50vh-84px/2)] md:translate-y-[calc(-84px+50vh-84px/2)]' // Centered when not authenticated
      ...
      >
        <div className={`w-full px-[20px] md:px-[30px] transition-all duration-[2000ms] ease-out ${!isAuthenticated && isProtectedDomain
          ...
          <FloatingBarContainer className='p-[5px] md:p-[5px] md:pl-[6px] md:py-[5px] !rounded-[27px]'>
            {!isAuthenticated && isProtectedDomain ? (
              // Auth UI - keep the same layout as normal bar
              <div className='w-full flex flex-col md:flex-row'>
                <div className={`flex items-center w-full gap-x-[5px] md:gap-x-[5px] z-0 pointer-events-auto`}>
                  ...
                  {/* Email Input (same position as search bar) */}
                  <div
                    className={`flex-1 min-w-0 relative h-[44px] md:ml-[15px] transition-[margin] duration-200`}
                  >
                    <EmailAuthInline />
                  </div>
                </div>
              ...
              <div className='w-full flex flex-col md:flex-row'>
                ...
                <div className={`flex items-center w-full gap-x-[5px] md:gap-x-[5px] z-0 pointer-events-auto`}>
                  ...
                  {/* Desktop - Home Button */}
                  <div className={`relative hidden md:flex items-center h-[44px] w-[58.87px] pl-[8px] pb-[2px] gap-x-[15px] ${isSidebarOpen ? "md:w-[245px] justify-between" : "md:w-[71.15px] justify-start"} transition-all duration-sidebar ease-sidebar`}>
                    {AUTH_SUBDOMAIN && (
                      <div className={`absolute hidden md:block heading-small-xs z-[10] top-0 left-0 transform ${isSidebarOpen ? "translate-y-[2px] translate-x-[55px] rotate-[0deg] scale-100" : "translate-y-[3px] translate-x-[-11px] rotate-[-45deg] scale-[0.8]"} transition-all duration-sidebar ease-sidebar`}>
                        {AUTH_SUBDOMAIN}.
                      </div>
                    )}
                    <GTPLogoOld />
                    ...
                    <SearchContainer>
                      <SearchBar
                        ...
                        onFocus={() => {
                          ...
                          const noResultsShowing = totalResults === 0;
                  ...
                  />
                  {!IS_PRODUCTION && !AUTH_SUBDOMAIN && (
                    <DarkModeToggleButton />
                  )}
                  {/* Logout button for protected domains */}
                  {isProtectedDomain && isAuthenticated && (
                      <FloatingBarExpanableLabelButton
