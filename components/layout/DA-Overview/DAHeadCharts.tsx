...
export default function DAHeadCharts({ selectedTimespan, isMonthly, data }: { selectedTimespan: string, isMonthly: boolean, data: AllDAOverview }) {
  ...
  return (
    <Carousel
      ariaId="da-overview"
      heightClass="h-[calc(232px+12px+15px)]" // height of the slides + height of the dots + distance from the slides to the dots
      minSlideWidth={350}
      maxSlidesPerView={3}
      pagination="dots"
      arrows={false}
      ...
      {Object.keys(data.metrics).filter(() => selectedTimespan !== "1d").reverse()
        .map((metricKey, i) => {
          ...
          return (
            ...
            <div key={metricKey + "_slide"} className={`select-none relative flex flex-col w-full overflow-hidden h-[232px] bg-color-bg-default rounded-2xl group
                ${selectedTimespan === "1d" ? "hidden" : "flex flex-col"}`}>
              <Link
              ...
              <div className="absolute numbers-lg top-[17px] right-[30px] numbers">
                ...
                }).format(getSumDisplayValue(data.metrics[metricKey]))
                }
                {is_fees ? "" : " GB"}
              </div>
