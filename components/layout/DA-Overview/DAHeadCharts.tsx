...
export default function DAHeadCharts({selectedTimespan, isMonthly, data}: {selectedTimespan: string, isMonthly: boolean, data: AllDAOverview}) {
    ...
    return (
      <Container>
    <div className={`wrapper  w-full mt-[15px] transition-height duration-500 relative overflow-hidden h-[232px] mb-[30px]`}> 
      <Splide
        options={{
          gap: "15px",
          autoHeight: true,
          ...
          breakpoints: {
            ...
            1100: {
              perPage: selectedTimespan === "1d" ? 1 : 2,
              drag: true,
            },
            1250: {
              perPage: selectedTimespan === "1d" ? 1 : 2,
            ...
            1450: {
              perPage: selectedTimespan === "1d" ? 1 : isSidebarOpen ? 2 : 3,
              drag: isSidebarOpen ? true : false,
            },
            1600: {
              perPage: selectedTimespan === "1d" ? 1 : 3,
              drag: false,
            },
            6000: {
              perPage: selectedTimespan === "1d" ? 1 : 3,
      ...
      >
        ...
        {Object.keys(data.metrics).filter(() => {return selectedTimespan !== "1d"}).reverse()
            .map((metricKey, i) => { 
            ...
            <SplideSlide key={metricKey + "_splide"}>       
            ...
            <div className="splide__arrows relative hidden  bottom-[103px]   md:-mt-0">
              <button className="splide__arrow splide__arrow--prev absolute hidden rounded-full text-forest-400 bg-white dark:bg-forest-700 mr-[5px]  !w-5 md:!w-8 !h-5 md:!h-8">
                <Icon
                  icon="feather:chevron-right"
                  className="w-3 h-3 md:w-6 md:h-6 z-50"
                />
              </button>
              <button className="splide__arrow splide__arrow--next rounded-full hidden text-forest-400 bg-white dark:bg-forest-700 -mr-2 md:-mr-14 !w-5 md:!w-8 !h-5 md:!h-8">
                <Icon
                  icon="feather:chevron-right"
                  className="w-3 h-3 md:w-6 md:h-6 z-50"
                />
