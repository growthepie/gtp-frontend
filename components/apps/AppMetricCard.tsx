...
export interface AppMetricCardProps {
  ...
  /** Latest value */
  value: number;
  /** Value one week ago — used to compute WoW % */
  prevValue: number;
  prefix?: string;
  suffix?: string;
...
// ─── Sparkline chart (mirrors MetricChart in MetricCards.tsx) ─────────────────
...
const SparklineChart = ({
  ...
  // Initialise circle at last data point
  useEffect(() => {
  ...
  const option = {
    ...
    tooltip: {
      ...
      axisPointer: {
        type: "line",
        lineStyle: { color: "#CDD8D3", width: 1, type: "solid" },
      },
    },
    series: [
      {
        ...
        >
          ...
          <div className="flex justify-between items-center gap-x-[10px] h-[12px]">
            ...
            <span className="numbers-xs text-color-text-primary font-medium">
              {prefix}
              {formatLargeNumber(customTooltip.value, 2)}
              {suffix}
            </span>
          </div>
        </div>
...
export default function AppMetricCard({
  ...
  const wowChange = prevValue !== 0 ? ((value - prevValue) / prevValue) * 100 : 0;
  const isPositive = wowChange >= 0;

  // Generate fake timestamps for the sparkline (last N days)
  const timestamps = sparkline.map((_, i) => {
    const d = new Date();
  ...
  return (
    ...
    >
      ...
      {/* Hover chevron — identical to MetricCards */}
