...
// How long a trending topic takes to arrive once it's opened. The two body types
// are tuned separately: a chart topic wipes in over CHART_REVEAL_DURATION_MS (see
...
const LandingEventsCardContent = ({ eventData }: { eventData: ResolvedEventExample }) => {
  ...
  // spread so the last one finishes at TILES_REVEAL_DURATION_MS. Pure CSS: the
  // animation runs on mount, which is when the tiles first render, so no timers or
  // per-frame state are involved.
  const prefersReducedMotion = useMediaQuery("(prefers-reduced-motion: reduce)");
  const tileStaggerMs = resolvedCards.length > 1
    ? Math.max(0, TILES_REVEAL_DURATION_MS - TILE_FADE_MS) / (resolvedCards.length - 1)
    : 0;
...
// first data point to the most recent — over CHART_REVEAL_DURATION_MS. Same mechanism
...
const LandingEventsChartContent = ({ eventData, onInteract }: { eventData: ResolvedEventExample; onInteract: () => void }) => {
  ...
  useLayoutEffect(() => {
    ...
    const step = (now: number) => {
      ...
      const t = Math.min((now - startWall) / CHART_REVEAL_DURATION_MS, 1);
      if (t < 1) {
        setRevealProgress(t);
