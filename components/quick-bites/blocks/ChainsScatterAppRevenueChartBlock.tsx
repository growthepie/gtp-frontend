...
export const ChainsScatterAppRevenueChartBlock: React.FC<ChainsScatterAppRevenueChartBlockProps> = ({ chartTitle }) => {
  ...
  const chainKeys = useMemo(() => {
    ...
    return Object.keys(masterData.chains).filter(key => {
      ...
      // Only include chains that support both daa and app_revenue metrics
      return chain.supported_metrics?.includes('daa') && chain.supported_metrics?.includes('app_revenue');
    });
  }, [masterData]);

  // Fetch active addresses (DAA) and App Revenue for all chains
  useEffect(() => {
    ...
    const fetchAllData = async () => {
      ...
      try {
        ...
        const appRevenueUrls = chainKeys.map(key => `https://api.growthepie.com/v1/metrics/chains/${key}/app_revenue.json`);
        
        const [daaResults, appRevenueResults] = await Promise.all([
          Promise.all(
        ...
        setAppRevenueDataArray(appRevenueResults);
      } catch (error) {
        console.error('Error fetching data:', error);
        setDaaDataArray([]);
        setAppRevenueDataArray([]);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const scatterData = useMemo(() => {
    if (!masterData?.chains || !daaDataArray.length || !appRevenueDataArray.length) return [];
    ...
    chainKeys.forEach((chainKey, index) => {
      ...
      const appRevenueData = appRevenueDataArray[index];
      
      if (!daaData || !appRevenueData) return;
      ...
      // Get app revenue (last_30d) - data array is [usd, eth], we use USD (index 0)
      const appRevenue30d = appRevenueData.details?.summary?.last_30d?.data?.[0] ?? appRevenueData.summary?.last_30d?.data?.[0];
      const appRevenue = appRevenue30d !== undefined ? appRevenue30d : null;
      
      if (activeAddresses === null || appRevenue === null) return;
    ...
    return top10Chains;
  }, [masterData, daaDataArray, appRevenueDataArray, chainKeys, theme]);
  ...
  const chartData = useMemo((): ChartSeriesData[] => {
    ...
    return scatterData.map(chain => ({
      ...
      type: 'scatter',
      color: chain.color,
      data: [[chain.activeAddresses, chain.appRevenue]]
    }));
  }, [scatterData]);
  ...
  const trendlineData = useMemo(() => {
    ...
    const trendlinePoints = activeChains.map(chain => [chain.activeAddresses, chain.appRevenue] as [number, number]);
  ...
  return (
    <div className="my-8">
      <ChartWrapper
        ...
        ratioTitle="App Revenue per Address"
        xAxisLabel="Active Addresses"
        yAxisLabel="App Revenue"
      />
      <figcaption className="text-center text-xs mt-2 text-forest-700 dark:text-forest-400 italic">
        Scatter plot showing the relationship between active addresses (x-axis) and app revenue (y-axis) for the top 10 chains by active addresses (last 30 days).<br />Trendline shows the correlation for selected chains.
      </figcaption>
    </div>
  );
};
