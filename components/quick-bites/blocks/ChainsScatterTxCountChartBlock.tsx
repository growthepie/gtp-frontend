...
export const ChainsScatterTxCountChartBlock: React.FC<ChainsScatterTxCountChartBlockProps> = () => {
  ...
  const scatterData = useMemo(() => {
    ...
    return top10Chains;
  }, [masterData, daaDataArray, txCountDataArray, chainKeys, theme]);
  ...
  return (
    <div className="my-8">
      <ChartWrapper
        ...
        ratioTitle="Transactions per Address"
      />
      <figcaption className="text-center text-xs mt-2 text-forest-700 dark:text-forest-400 italic">
        Scatter plot showing the relationship between active addresses (x-axis) and transaction count (y-axis) for the top 10 chains by active addresses (last 30 days).<br />Trendline shows the correlation for selected chains.
      </figcaption>
    </div>
  );
};
