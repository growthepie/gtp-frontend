...
export const ChainsScatterRentPaidChartBlock: React.FC<ChainsScatterRentPaidChartBlockProps> = () => {
  ...
  const chainKeys = useMemo(() => {
    ...
    return Object.keys(masterData.chains).filter(key => {
      ...
      // Only include chains that support both daa and rent_paid metrics
      return chain.supported_metrics?.includes('daa') && chain.supported_metrics?.includes('rent_paid');
    });
  }, [masterData]);

  // Fetch active addresses (DAA) and Rent Paid for all chains
  useEffect(() => {
    ...
    const fetchAllData = async () => {
      ...
      try {
        ...
        const rentPaidUrls = chainKeys.map(key => `https://api.growthepie.com/v1/metrics/chains/${key}/rent_paid.json`);
        
        const [daaResults, rentPaidResults] = await Promise.all([
          Promise.all(
        ...
        setRentPaidDataArray(rentPaidResults);
      } catch (error) {
        console.error('Error fetching data:', error);
        setDaaDataArray([]);
        setRentPaidDataArray([]);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const scatterData = useMemo(() => {
    if (!masterData?.chains || !daaDataArray.length || !rentPaidDataArray.length) return [];
    ...
    chainKeys.forEach((chainKey, index) => {
      ...
      const rentPaidData = rentPaidDataArray[index];
      
      if (!daaData || !rentPaidData) return;
      ...
      // Get rent paid (last_30d) - data array is [usd, eth], we use USD (index 0)
      const rentPaid30d = rentPaidData.details?.summary?.last_30d?.data?.[0] ?? rentPaidData.summary?.last_30d?.data?.[0];
      const rentPaid = rentPaid30d !== undefined ? rentPaid30d : null;
      
      if (activeAddresses === null || rentPaid === null) return;
    ...
    return top10Chains;
  }, [masterData, daaDataArray, rentPaidDataArray, chainKeys, theme]);
  ...
  const chartData = useMemo((): ChartSeriesData[] => {
    ...
    return scatterData.map(chain => ({
      ...
      type: 'scatter',
      color: chain.color,
      data: [[chain.activeAddresses, chain.rentPaid]]
    }));
  }, [scatterData]);
  ...
  const trendlineData = useMemo(() => {
    ...
    const trendlinePoints = activeChains.map(chain => [chain.activeAddresses, chain.rentPaid] as [number, number]);
  ...
  return (
    <div className="my-8">
      <ChartWrapper
        ...
        ratioTitle="Rent Paid per Address"
        xAxisLabel="Active Addresses"
        yAxisLabel="Rent Paid to L1"
      />
      <figcaption className="text-center text-xs mt-2 text-forest-700 dark:text-forest-400 italic">
        Scatter plot showing the relationship between active addresses (x-axis) and rent paid to L1 (y-axis) for the top 10 chains by active addresses (last 30 days).<br />Trendline shows the correlation for selected chains.
      </figcaption>
    </div>
  );
};
