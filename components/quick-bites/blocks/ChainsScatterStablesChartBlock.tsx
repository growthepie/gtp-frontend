...
interface ChainsScatterStablesChartBlockProps {
  chartTitle?: string;
}
...
export const ChainsScatterStablesChartBlock: React.FC<ChainsScatterStablesChartBlockProps> = ({ chartTitle }) => {
  const { theme } = useTheme();
  const [daaDataArray, setDaaDataArray] = useState<(MetricData | null)[]>([]);
  const [stablesDataArray, setStablesDataArray] = useState<(MetricData | null)[]>([]);
  ...
  const chainKeys = useMemo(() => {
    ...
    return Object.keys(masterData.chains).filter(key => {
      ...
      // Only include chains that support both daa and stables_mcap metrics
      return chain.supported_metrics?.includes('daa') && chain.supported_metrics?.includes('stables_mcap');
    });
  }, [masterData]);

  // Fetch active addresses (DAA) and Stablecoin Supply for all chains
  useEffect(() => {
    ...
    const fetchAllData = async () => {
      ...
      try {
        ...
        const stablesUrls = chainKeys.map(key => `https://api.growthepie.com/v1/metrics/chains/${key}/stables_mcap.json`);
        
        const [daaResults, stablesResults] = await Promise.all([
          Promise.all(
        ...
        setStablesDataArray(stablesResults);
      } catch (error) {
  ...
  const scatterData = useMemo(() => {
    if (!masterData?.chains || !daaDataArray.length || !stablesDataArray.length) return [];
    ...
    chainKeys.forEach((chainKey, index) => {
      ...
      const stablesData = stablesDataArray[index];
      
      if (!daaData || !stablesData) return;
      ...
      // Get stablecoin supply (last_30d) - data array is [usd, eth], we use USD (index 0)
      const stables30d = stablesData.details?.summary?.last_30d?.data?.[0] ?? stablesData.summary?.last_30d?.data?.[0];
      const stablesSupply = stables30d !== undefined ? stables30d : null;
      
      if (activeAddresses === null || stablesSupply === null) return;
    ...
    return top10Chains;
  }, [masterData, daaDataArray, stablesDataArray, chainKeys, theme]);
  ...
  const chartData = useMemo((): ChartSeriesData[] => {
    ...
    return scatterData.map(chain => ({
      ...
      type: 'scatter',
      color: chain.color,
      data: [[chain.activeAddresses, chain.stablesSupply]]
    }));
  }, [scatterData]);
  ...
  const trendlineData = useMemo(() => {
    ...
    const trendlinePoints = activeChains.map(chain => [chain.activeAddresses, chain.stablesSupply] as [number, number]);
  ...
  return (
    <div className="my-8">
      <ChartWrapper
        ...
        ratioTitle="Stablecoins per Address"
        xAxisLabel="Active Addresses"
        yAxisLabel="Stablecoin Supply"
      />
      <figcaption className="text-center text-xs mt-2 text-forest-700 dark:text-forest-400 italic">
        Scatter plot showing the relationship between active addresses (x-axis) and stablecoin supply (y-axis) for the top 10 chains by active addresses (last 30 days).<br />Trendline shows the correlation for selected chains.
      </figcaption>
    </div>
  );
};
