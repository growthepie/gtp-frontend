...
interface ChainsScatterFeesChartBlockProps {
  chartTitle?: string;
}
...
export const ChainsScatterFeesChartBlock: React.FC<ChainsScatterFeesChartBlockProps> = ({ chartTitle }) => {
  const { theme } = useTheme();
  const [daaDataArray, setDaaDataArray] = useState<(MetricData | null)[]>([]);
  const [feesDataArray, setFeesDataArray] = useState<(MetricData | null)[]>([]);
  ...
  const chainKeys = useMemo(() => {
    ...
    return Object.keys(masterData.chains).filter(key => {
      ...
      // Only include chains that support both daa and fees metrics
      return chain.supported_metrics?.includes('daa') && chain.supported_metrics?.includes('fees');
    });
  }, [masterData]);

  // Fetch active addresses (DAA) and Fees for all chains
  useEffect(() => {
    ...
    const fetchAllData = async () => {
      ...
      try {
        ...
        const feesUrls = chainKeys.map(key => `https://api.growthepie.com/v1/metrics/chains/${key}/fees.json`);
        
        const [daaResults, feesResults] = await Promise.all([
          Promise.all(
  ...
  const scatterData = useMemo(() => {
    if (!masterData?.chains || !daaDataArray.length || !feesDataArray.length) return [];
    ...
    chainKeys.forEach((chainKey, index) => {
      ...
      const feesData = feesDataArray[index];
      
      if (!daaData || !feesData) return;
      ...
      // Get fees (last_30d) - data array is [usd, eth], we use USD (index 0)
      const fees30d = feesData.details?.summary?.last_30d?.data?.[0] ?? feesData.summary?.last_30d?.data?.[0];
      const fees = fees30d !== undefined ? fees30d : null;
      
      if (activeAddresses === null || fees === null) return;
    ...
    return top10Chains;
  }, [masterData, daaDataArray, feesDataArray, chainKeys, theme]);
  ...
  const chartData = useMemo((): ChartSeriesData[] => {
    ...
    return scatterData.map(chain => ({
      ...
      type: 'scatter',
      color: chain.color,
      data: [[chain.activeAddresses, chain.fees]]
    }));
  }, [scatterData]);
  ...
  const trendlineData = useMemo(() => {
    ...
    const trendlinePoints = activeChains.map(chain => [chain.activeAddresses, chain.fees] as [number, number]);
  ...
  return (
    <div className="my-8">
      <ChartWrapper
        ...
        width="100%"
        height={400}
        title={chartTitle || "Chain Revenue"}
        subtitle="30-day comparison for top 10 chains by active addresses"
        showXAsDate={false}
        disableTooltipSort={false}
        options={chartOptions}
        onFilterChange={handleFilterChange}
        ratioTitle="Chain Revenue per Address"
        xAxisLabel="Active Addresses"
        yAxisLabel="Chain Revenue"
      />
      <figcaption className="text-center text-xs mt-2 text-forest-700 dark:text-forest-400 italic">
        Scatter plot showing the relationship between active addresses (x-axis) and chain revenue (y-axis) for the top 10 chains by active addresses (last 30 days).<br />Trendline shows the correlation for selected chains.
      </figcaption>
    </div>
  );
};
