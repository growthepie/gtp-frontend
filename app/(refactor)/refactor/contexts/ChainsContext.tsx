"use client";
import { BlockspaceURLs, ChainBlockspaceURLs, ChainURLs, MasterURL } from "@/lib/urls";
import { ChainData, ChainOverviewResponse } from "@/types/api/ChainOverviewResponse";
import { ChainRanking, ChainResponse, HottestContractData } from "@/types/api/ChainResponse";
import { ChainDatum, FeesTableResponse } from "@/types/api/Fees/Table";
import { MasterResponse, ChainInfo, Stack, L2BeatStage, BlockExplorers } from "@/types/api/MasterResponse";
...
export const ChainsProvider = ({ children, chainKey }: { children: React.ReactNode, chainKey: string }) => {
  const { data: masterData, isLoading: masterLoading } = useSWR<MasterResponse>(MasterURL);
  const { data: chainData, isLoading: chainLoading } = useSWR<ChainResponse>(ChainURLs[chainKey]);
  const { data: feeData, isLoading: feeLoading } = useSWR<FeesTableResponse>("https://api.growthepie.xyz/v1/fees/table.json");
  const { data: overviewData, isLoading: overviewLoading } = useSWR<ChainData>(ChainBlockspaceURLs[chainKey]);

  const [sectionHead, setSectionHead] = useState<SectionHead | undefined>(undefined);

  const [compareChain, setCompareChain] = useState<string | undefined>(undefined);
      ...
      menu: {
        ...
        bridge_url: masterData.chains[chainKey].rhino_listed ?
          masterData.chains[chainKey].rhino_naming ?
            ...
            "https://app.rhino.fi/bridge/?refId=PG_GrowThePie" : undefined,
      },
      background: {
