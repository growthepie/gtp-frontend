...
const Chain = ({ params }: { params: any }) => {
  ...
  const [pageName, setPageName] = useState(String(chain).charAt(0).toUpperCase() + String(chain).slice(1));
      ...
      <div className="ml-12 mr-14">
        <div className="flex justify-between items-center">
            ...
            <div className="flex gap-x-10 h-10">
              <Link href={(chainData.block_explorer)} className="flex justify-between text-white dark:bg-[#2A3433] dark:text-[#CDD8D3] bg-blue-600 w-44 py-0 rounded-full w-168px pl-4 pr-6">
                <LinkIcon className="h-4 w-4 self-center" />
                <p className="self-center font-semibold">Block Explorer</p>
              </Link>
              
              <Link href={(chainData.twitter)} className="flex justify-between text-white dark:bg-[#2A3433]  dark:text-[#CDD8D3] bg-blue-600 w-40 py-0 rounded-full w-168px pl-4 pr-6">
                <AtSymbolIcon className="h-4 w-4 self-center" />
                <p className="self-center font-semibold">{chainData.twitter.split( "https://twitter.com/")}</p>
              </Link>
              
              <Link href={(chainData.website)} className="flex justify-between text-white dark:bg-[#2A3433]  dark:text-[#CDD8D3] bg-blue-600 w-32 py-0 rounded-full w-168px pl-4 pr-6">
