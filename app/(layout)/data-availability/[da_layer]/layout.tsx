...
import { MetricItem, metricItems, daMetricItems } from "@/lib/metrics";

type Props = {
  params: { da_layer: string };
};
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  if (
    !params.da_layer ||
    daMetricItems.find((item) => item.urlKey === params.da_layer) === undefined
  ) {
    track("404 Error", {
      location: "404 Error",
      page: "/data-availability/" + params.da_layer,
    });
    return notFound();
  }

  const option = daMetricItems.find((item) => item.urlKey === params.da_layer);
  ...
  if (option) {
    ...
    return {
      ...
      openGraph: {
        images: [
          {
            url: `https://api.growthepie.xyz/v1/og_images/data-availability/${params.da_layer}.png?date=${dateString}`,
            width: 1200,
            height: 627,
            alt: "growthepie.xyz",
          },
...
export default async function Layout({
  ...
  const url = MetricsURLs[params.da_layer];

  const pageData = daMetricItems.find((item) => item.urlKey === params.da_layer)
    ?.page ?? {
    title: "",
    description: "",
    icon: "",
              ...
              >
                <div
                  className={`flex items-center justify-center p-[1px] bg-[linear-gradient(144.58deg,#FE5468_20.78%,#FFDF27_104.18%)] rounded-full  ${
                    params.da_layer === "transaction-costs" ? "flex" : "hidden"
                  }`}
                >
